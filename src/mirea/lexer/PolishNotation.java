...
public class PolishNotation {
    ...
    public List<String> translateToPolishNotation() {
        ...
        while(currentList.size()!=0) {
            ...
            if(lexemeList.get(0).getTerminal().getIdentifier()=="WHILE_KEYWORD") {

            }
            convertExpression(lexemeList);
        }
        return polishNotation;
    }

    public void convertWhile(List<Lexeme> lexemeList) {
        lexemeList.remove(0);
    }

    public void convertExpression(List<Lexeme> lexemeList) {
        ...
        while(lexemeList.size()!=1) {   //Последний символ ;
            ...
            if(lexemeList.get(0).getTerminal().getIdentifier()=="VAR" || lexemeList.get(0).getTerminal().getIdentifier()=="NUMBER") {
                polishNotation.add(lexemeList.get(0).getValue());
                lexemeList.remove(0);
            }
